...
#store panel exon coordinates
open(EXON, "/isilon/Analysis/onco/indexes/hg38/panel_exon_coor.txt") || die "Cannot open IN: $!";
my %exon;
while( my $record=<EXON> )
{
    chomp $record;
    $record =~ s/\r|\n//g;
...
if (-e $filename2)
{
	...
	{
	    my @line = split("\t", $record);
	    $pct_exp = sprintf("%.3f", $line[2]*100+0.0000000001);
	}
	elsif ($record =~ /^Percentage of phix reads/)
	{
	    $tag++;
	    my @line = split("\t", $record);
	    $pct_phix = sprintf("%.1f", $line[2]+0.0000000001);
	}
	elsif ($record =~ /^Percentage of reads binned to samples/)
	{
	    $tag++;
	    my @line = split("\t", $record);
	    $pct_binned = sprintf("%.1f", $line[2]+0.0000000001);
	}
    }
    close IN;
    if ($tag == 0)
...
foreach my $name (sort {$a cmp $b} @files)
{
	...
	if (-e $file_name)
	{
		...
		{
		    $adapter1 = sprintf("%.2f", $1+0.0000000001);
		}
	    }
	    close IN;
	...
	if (-e $file_name)
	{
		...
		if ($record =~ /\>\>Adapter Content.*\n84\s+(\S+)\s+/s)
		{
		    $adapter2 = sprintf("%.2f", $1+0.0000000001);
		}
	    }
	    close IN;
	    $/ = $save_input_separator;
	...
	if (-e $file_name)
	{
		...
		{
                    my @line = split("\t", $record);
                    $mapped = sprintf("%.1f", $line[2]+0.0000000001);
		}
		elsif ($record =~ /^Sorted BAM\: Fraction of properly paired aligned reads\s+number/)
		{
                    my @line = split("\t", $record);
                    $paired = sprintf("%.1f", $line[2]+0.0000000001);
		}
            }
            close IN;
	}
	...
	if (-e $file_name)
        {
            ...
            while( my $record=<IN> )
            {
                ...
                {
                    my @line = split("\t", $record);
                    $ontarget = sprintf("%.1f", $line[2]+0.0000000001);
                }
	    }
            close IN;
	}
	...
	if (-e $file_name)
	{
		...
		{
		    my @line = split("\t", $record);
		    $singleton = sprintf("%.2f", $line[2]+0.0000000001);
		}
		...
		elsif ($record =~ /^BCDEDUP_EFFSTATS\: duplex efficiency\s+number/)
		{
		    ...
		    $duplex_eff = sprintf("%.1f", $line[2]*100+0.0000000001);
                ...
                {
                    my @line = split("\t", $record);
                    $ge = sprintf("%.3f", $line[2]+0.0000000001);
                }
	    }
            close IN;
	}
	...
	#metrics: Mean_Fragment_Length
        ...
        if (-e $file_name)
        {
            ...
            while( my $record=<IN> )
            {
                ...
                {
                    my @line = split("\t", $record);
                    $frag_len = sprintf("%.1f", $line[2]+0.0000000001);
                }
	    }
            close IN;
        }
	...
	if (-e $file_name)
	{
		...
		{
		    my @line = split("\t", $record);
		    $error_pos = sprintf("%.1f", $line[2]+0.0000000001);
		}
		elsif ($record =~ /^BGPOLISHED_ERRRATES\: Error rate\s+number/)
                {
                    my @line = split("\t", $record);
                    $error_rate = sprintf("%.6f", $line[2]*100+0.0000000001);
                }
	    }
            close IN;
	}
        ...
        if (-e $file_name)
	{
		...
		{
                    my @line = split("\t", $record);
                    $fold2 = sprintf("%.2f", $line[2]+0.0000000001);
		}
		...
		{
                    my @line = split("\t", $record);
                    $ratio = sprintf("%.2f", $line[2]+0.0000000001);
		}
            }
            close IN;
	}
...
sub pct_depth_2000
{
    ...
    my $dep2000 = sprintf("%.1f", $over2000/$total*100+0.0000000001);
    return($dep2000);
}
