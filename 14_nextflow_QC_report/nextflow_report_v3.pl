...
print OUT "Sample\tLane_Total_Reads\tReads_Binned_to_Samples\t%PhiX_Reads\t%Expected_SID\t%Adapter_R1\t%Adapter_R2\tSample_Total_Reads\t%Reads_Mapped\t%Paired_Reads_Mapped\t%Reads_On-Target\tNon-dedup_Depth\tDedup_Depth\tDuplication_Rate\tMean_Fragment_Length\t%Genotyping_LOD\t%Error_Free_Positions\t%Error_Rate\t%Barcode_Singleton\tPeak_Family_Size\tFold_Overseq\t%Duplex_Family\t%Duplex_Reads\tInput_Mass(ng)\tGE_Recovery_Rate\t%Bases_in_2-fold_Range\t%Bases_in_10-fold_Range\tRatio_90th-pct/10th-pct\n";
