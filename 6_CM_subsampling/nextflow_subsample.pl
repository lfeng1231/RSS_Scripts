...
my $subdir2= "$dir/$folder/demux";
system("rm -rf $subdir");
system("mkdir $subdir");
system("rm -rf $subdir2");
system("mkdir $subdir2");
system("ln -s $dir/*R1*/*.fastq $subdir2");
system("ln -s $dir/*R2*/*.fastq $subdir2");
...
foreach my $name (@files)
{
	...
	my $count = `wc -l $subdir2/$r1 | cut -d ' ' -f 1`;
	my $count2 = $count/2;
	print OUT "$r1\t$count2\n";
	...
	system("date");
	system("/remote/RSU/sw/seqtk/06222015/seqtk sample -s $seed $subdir2/$r1 $read_pair > $subdir/$sr1");
	system("/remote/RSU/sw/seqtk/06222015/seqtk sample -s $seed $subdir2/$r2 $read_pair > $subdir/$sr2");
	system("date");
